import pandas as pd


def generateRank(indicator):
    if indicator == 'Export':
        abbr = 'EXP'
    elif indicator == 'Import':
        abbr = 'IMP'
    elif indicator == 'NetTrade':
        abbr = 'NTRADE'
    file = pd.read_csv('export.csv')
    predictValue = pd.DataFrame(columns=['Country', 'Time', indicator])
    newFile = pd.DataFrame(file.loc[(file['MEASURE'] == 'BLN_USD') &
                                    (file['SUBJECT'] == abbr)])

    i = 0
    index = 1
    location = pd.Series(0, index=pd.RangeIndex(0, 150))
    f_location = newFile['LOCATION']
    location[0] = f_location.iloc[0]
    locationIndex = location[0]
    for num in f_location:
        if num == locationIndex:
            i += 1
            continue
        else:
            location[index] = num
            locationIndex = num
            i += 1
            index += 1

    location = location[location != 0]
    for country in location:
        f = pd.DataFrame(newFile.loc[(newFile['LOCATION'] == country)])
        f.index = f['TIME'].values
        if not ('2021-Q3' in f.index):
            continue
        f_new = pd.DataFrame(f.loc['2018-Q1':'2021-Q3'])
        f_new = f_new[['LOCATION', 'Value']]
        f_Value = f_new['Value']
        rate = pd.Series(index=pd.RangeIndex(0, 15))
        i = 0
        for index in f_Value:
            if i <= 13:
                former = float(f_Value[i])
                latter = float(f_Value[i + 1])
                rate[i] = float((latter - former) / former)
                i += 1
        newValue = float(f_Value['2021-Q3']) * (1 + rate[13])
        f_new.loc['2021-Q4'] = [country, newValue]
        predictValue.loc[country] = [country, '2021-Q4', newValue]

    rankGDP = predictValue.sort_values(indicator, ascending=False)
    rankGDP.index = range(1, len(predictValue.index) + 1)
    numRank = input("The total country for rank is " + str(
        len(predictValue.index)) + ". Please enter the number you want to display for TOPN: ")
    print(rankGDP.iloc[0:int(numRank)])
    print("\n")


num_Indicator = input(
    "Please select the number of indicator:" + "\n" + "1 Export" + "\n" + "2 Import" + "\n" + "3 Net Trade" + "\n" + "4 Exit" + "\n")
while num_Indicator != str(4):
    if num_Indicator == str(1):
        generateRank('Export')
    elif num_Indicator == str(2):
        generateRank('Import')
    elif num_Indicator == str(3):
        generateRank('NetTrade')
    num_Indicator = input(
        "Please select the number of indicator:" + "\n" + "1 Export" + "\n" + "2 Import" + "\n" + "3 Net Trade" + "\n" + "4 Exit" + "\n")

exit(0)
